...
can check the frequencies of the outcome levels to get the idea of their
distribution.
...
We will also remove the unnecessary descriptive variables (serial IDs, timestamps).
...
We split the training dataset in two parts: 70% will be used for model training,
and the remaining 30% will be kept apart as a validation dataset, to compare the
performance of different algorithms.
Also, we will pre-process the data by centering and scaling all variables.
...
in the dataset as predictors. We will try different algorithms to compare their 
performance. We will leverage the *caret* package to train them by using
*10-fold cross-validation*.

```{r models, cache = TRUE, hide = TRUE}
models <- list()
fitControl <- trainControl(method = "cv",
                           number = 10) # >change to 10<

models[["lda"]] <-
  train(classe ~ ., data = trainSetPre, method = "lda", trControl = fitControl)
models[["gbm"]] <-
   train(classe ~ ., data = trainSetPre, method = "gbm", trControl = fitControl)
models[["rpart"]] <-
...
will yeald the best overall accuracy will be chosen.
...
From the table above, Random Forest is the contest winner having achieved an
impressive accuracy of `r best[1,2]` on validation data, which is our estimate
...
```{r best}
testPred <-
  predict(models[[as.numeric(row.names(best))]],
          validSetPre) # predictions from the best model
...
```{r varImp, cache = TRUE}
varImp(finalModel) # variables importance in the final model

varImp <- varImp(finalModel)$importance
...
Having selected randomForest as the best algorithm in this case, we proceed to
